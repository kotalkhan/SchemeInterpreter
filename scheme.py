...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 4
        #validate_procedure(expr.first)
        pair_args = expr.map(lambda one_expression: scheme_eval(one_expression, env))
        return scheme_apply(pair_args.first, pair_args.rest, env)
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    while expressions is not nil:
        final_value  = scheme_eval(expressions.first, env)
        expressions = expressions.rest
    return final_value    # END PROBLEM 7
